...
# Comando: Abraçar outro membro (diversão)
@bot.command()
...
async def perfil(ctx, member: discord.Member = None):
    ...
    fonte_texto = ImageFont.truetype("arial.ttf", 30)
    fonte_destaque = ImageFont.truetype("arial.ttf", 40)
...
# Substitua o token fixo pela variável de ambiente
# Carrega o token do arquivo de configuração
token = configuracoes.get("token")

if not token:
    raise ValueError("❌ O token do bot não foi encontrado no arquivo config.json. Adicione-o e tente novamente.")
